...
# Source of the defined functions:
...
def process(ip_image):
	...
	cont = cv2.convertScaleAbs(ip_image, alpha=2, beta=0.5)

	pic = np.float32(cont)/255.0
	...
	if detect_aruco:
		...
		pic = mark_Aruco(deconvulted,detect_aruco)

		robot_state = calculate_Robot_State(pic,detect_aruco)

		cv2.imwrite(generated_folder_path+"/"+'aruco_with_id.png',pic)
